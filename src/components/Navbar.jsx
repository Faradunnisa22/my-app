    ...
    return (
        <header className='w-full bg-white md:bg-transparent fixed top-0 left-0 right-0'>
            <nav className={`py-4 lg:px-14 px-4 ${isSticky ? "sticky top-0 left-0 right-0 border bg-white duration-300" : ""}`}>
                <div className='flex md:mx-40 justify-between items-center text-base gap-8 '>
                    <a href="/" className=' flex items-center space-x-3'><img src={logo} alt="" className='inline-block items-center' /> </a>
                ...
                <div className={`space-y-4 px-4 mt-16 py-7 bg-red-400 ${isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden"}`}>
                    {
                        navItems.map(({ link, path }) =>
                            <Link
                                to={path}
                                ...
                                className='block text-base text-white
                                 bg-red-400 hover:text-brandPrimary first:font-medium'
                            >
                                {link}</Link>)
                    }
                </div>
